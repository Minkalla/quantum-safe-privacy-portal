...
fn benchmark_mlkem_operations(c: &mut Criterion) {
    ...
    group.bench_function("decapsulation", |b| {
        b.iter_custom(|iters| {
            ...
            for _ in 0..iters {
                let operation_start = Instant::now();
                let _result = black_box(decapsulate(keypair.private_key.expose_secret(), &ciphertext));
                let duration = operation_start.elapsed();
...
fn benchmark_combined_operations(c: &mut Criterion) {
    ...
    group.bench_function("full_mlkem_cycle", |b| {
        b.iter(|| {
            ...
            let (ciphertext, _shared_secret) = black_box(encapsulate(&keypair.public_key, message).unwrap());
            let _decrypted = black_box(decapsulate(keypair.private_key.expose_secret(), &ciphertext));
        })
    });
...
fn save_baseline_measurements(measurements: &[(String, Duration)], algorithm: &str) {
    let report_path = format!("/tmp/pqc_performance/baselines/{}_baseline_measurements.txt", algorithm);
    ...
    if let Ok(mut file) = OpenOptions::new()
        ...
        writeln!(file, "\n=== {} Performance Baseline Measurements ===", algorithm.to_uppercase()).ok();
        writeln!(file, "Generated: {}", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC")).ok();
        ...
        for (operation, duration) in measurements {
            grouped.entry(operation.clone()).or_insert_with(Vec::new).push(*duration);
        }
        ...
        for (operation, durations) in grouped {
            if !durations.is_empty() {
                ...
                let mean = Duration::from_nanos(
                    sorted_durations.iter().map(|d| d.as_nanos()).sum::<u128>() as u64 / sorted_durations.len() as u64
                );
                ...
                writeln!(file, "  Max Duration: {:?}", sorted_durations[sorted_durations.len() - 1]).ok();
                writeln!(file, "").ok();
            }
        }
    }
