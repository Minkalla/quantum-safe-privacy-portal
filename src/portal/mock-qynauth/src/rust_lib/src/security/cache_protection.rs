...
impl CacheProtection {
    ...
    pub fn flush_cache_lines(data: &[u8]) {
        unsafe {
            for chunk in data.chunks(64) { // 64-byte cache line size
...
mod cache_protection_tests {
    ...
    fn test_cache_safe_string_search() {
        assert!(CacheProtection::cache_safe_string_search("hello world", "world"));
        assert!(CacheProtection::cache_safe_string_search("hello world", "hello"));
        assert!(!CacheProtection::cache_safe_string_search("hello world", "xyz"));
